...
新增一个轻量的 `MapRoutePresentation` 值类型，包含 `routeID` 和零至两个 `MapNormalizedPoint` 控制点。控制点不进入规则 JSON、保存状态或网络协议。
...
可见路线与 44 pt 命中路线必须共享同一个 `CGPath`，避免看得到却点不到。路线标签使用同一几何的中点；为避免引入通用曲线求长算法，中点按对应 Bézier 在 `t = 0.5` 计算，再沿用当前 18 scene-unit 的上移偏移。
...
5. `GameMapScene.configure` 合并实时对局状态与静态展示几何。
6. `MapNodeFactory` 把左上原点归一化坐标转换为 SpriteKit 坐标，并生成共享的可见/命中曲线路径。
...
5. 更新路线命中测试，使用实际曲线在 `t = 0.5` 的点，而不是端点算术平均值。
6. 运行整个 `IndustrialCityBirminghamTests`，确保规则、交互、视口与命中区域没有回归。
7. 运行 `FriendsPlayableUITests/testRealFixtureLandscapeVisualEvidence`，导出新的横屏截图，与开发期参考图人工核对朝向、节点顺序、路线交叉和文字遮挡。
