...
6. 更新路线命中测试，使用实际曲线在 `t = 0.5` 的点，而不是端点算术平均值。
7. 运行整个 `IndustrialCityBirminghamTests`，确保规则、交互、视口与命中区域没有回归。
8. 运行 `FriendsPlayableUITests/testRealFixtureLandscapeVisualEvidence`，导出新的横屏截图，与开发期参考图人工核对朝向、节点顺序、路线交叉、两个乡村酒桶的位置和文字遮挡。
